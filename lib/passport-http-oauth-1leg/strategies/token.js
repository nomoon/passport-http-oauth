...
TokenStrategy.prototype.authenticate = function(req) {
  ...
  this._consumer(consumerKey, function(err, consumer, consumerSecret) {
    ...
    self._verify(accessToken, verified, consumer);
    ...
    function verified(err, user, tokenSecret, info) {
      ...
      if (self._validate) {
        ...
        // it so desires.
        self._validate(timestamp, nonce, user, function(err, valid, message) {
          if (err) { return self.error(err); }
          if (!valid) { return self.fail(self._challenge(message || 'nonce_timestamp_invalid')); }
